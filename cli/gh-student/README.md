...
| `gh student submit`                                | Snapshot the current branch and force-push it to the assignment repo's `main` branch (hardcoded for now), after fetching the instructor's `.gitignore` (if present) and `.github/` from the template. Reads `.classroom50.yml` for the source repo and branch. |
