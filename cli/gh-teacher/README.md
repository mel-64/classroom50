...
- `helpers.go` — cross-cutting CLI helpers: `validateShortName(name, label)` enforces `shortNamePattern` consistently for classroom short-names, assignment slugs, and autograder names; `validateOrgName` and `parseOrgClassroom` validate command arguments up front. (The `*api.HTTPError` → `StatusCode` predicate moved to `internal/cliutil` as `cliutil.IsHTTPStatus`.)
