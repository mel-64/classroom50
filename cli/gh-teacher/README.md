...
The default autograder shim + orchestrator scaffolded into each classroom by `gh teacher classroom add` live under `autograders/`:
- `autograders/default.yaml` — the workflow shim dropped at `<classroom>/autograders/default.yaml`
- `autograders/autograde.py` — the runtime orchestrator dropped at `<classroom>/autograders/autograde.py`; fetched by the autograde-runner reusable workflow at workflow runtime on every submission (the shim only `uses:` the runner — it never performs Pages fetches itself)

Tests for the embedded Python files (`skeleton/dotgithub/scripts/collect_scores.py` and `autograders/autograde.py`) live under `skeleton_tests/` and `autograders_tests/` respectively. They run in CI via [`skeleton-scripts-ci.yaml`](../../.github/workflows/skeleton-scripts-ci.yaml); execute locally with `python3 -m pytest skeleton_tests/ autograders_tests/`.
...
- `autograder.go` — the embedded autograder shim and orchestrator (`defaultAutograderYAML`, `defaultAutogradePyScript` — both `//go:embed`), the `autograder` name validator (shares `shortNamePattern` with classroom/slug because the value flows into the same paths), and the contents-API existence probe `autograderExists` (used by `gh teacher assignment add --autograder` at write time). Covered by `autograder_test.go`.
