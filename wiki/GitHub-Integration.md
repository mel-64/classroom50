...
Org-level member defaults (base permission = "No permission", public-repo creation disabled, and private-repo creation enabled so `gh student accept` works) are no longer a manual step -- `gh teacher init` applies them via `PATCH /orgs/{org}` (see the REST table below), retrying each policy individually if the combined PATCH hits a plan-gated or enterprise-locked field. The fields GitHub rejects are the one case where you'd still flip them yourself at `https://github.com/organizations/<org>/settings/member_privileges`; init warns per rejected field with that link.
...
| PATCH | [`https://api.github.com/orgs/{org}`](https://docs.github.com/en/rest/orgs/orgs#update-an-organization) | Tighten org member defaults at `init` time: `default_repository_permission: "none"` + `members_can_create_public_repositories: false` + `members_can_create_private_repositories: true` (combined, with a per-field retry on 403/422) |
