...
7. **Drop `.classroom50.yaml` and `.github/workflows/autograde.yaml` in a single Tree commit** on the templated branch. The workflow body is the YAML fetched in step 3 — source-of-truth is the config repo, not the student CLI. The metadata records four blocks:
   ...
   - `autograde.{source,fetched_at}` — diagnostics: `source` records the resolved shim ref (e.g. `autograders/default.yaml`); `fetched_at` is the UTC timestamp of the fetch. Useful for debugging "which shim is in this student's repo right now?" without leaving the terminal.
...
5. **Re-fetch the autograder workflow shim body** (`https://<org>.github.io/classroom50/<classroom>/autograders/<name>.yaml`) and overwrite `.github/workflows/autograde.yaml`. The fetched body is the single source of truth; whatever the template's `.github/` fetch in step 3 brought along is replaced. `autograde.fetched_at` (UTC) and `autograde.source` in `.classroom50.yaml` are updated for diagnostics. The shim is intentionally stable — its only job is to `uses:` the autograde-runner reusable workflow in the teacher's config repo, and the **runner** is what fetches the actual orchestrator and tests from Pages at workflow runtime. So a stale shim still grades correctly against the latest teacher-side logic. (404 / malformed-YAML / empty-body errors are surfaced the same way as on accept — see §3 above.)
